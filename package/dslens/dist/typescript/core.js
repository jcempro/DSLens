...
function parseSelector(source) {
    if (source.length > LIMITS.queryLength)
...
class SelectorParser {
    ...
    parseStep() {
        if (this.consume('..'))
            return { kind: 'recursive', target: this.parseRecursiveTarget() };
        if (this.consume('.text()'))
            return { kind: 'text' };
        if (this.consume('.@'))
            return { kind: 'attribute', name: this.parseName() };
        if (this.consume('.['))
            return { kind: 'property', name: this.parseQuoted(']') };
        if (this.consume('.'))
    ...
    parseFilter() {
        this.filters += 1;
        if (this.filters > LIMITS.filters)
            throw new Error('limit');
        if (!this.consume('@'))
    ...
    parsePredicatePath() {
        ...
        while (this.cursor < this.source.length) {
            this.skipSpaces();
            if (/^(?:=|!=|>=|<=|>|<|\)\])/u.test(this.source.slice(this.cursor)))
                break;
            this.parseStep();
        }
    ...
    parseScalar() {
        ...
        if (token.length > LIMITS.literalLength)
            throw new Error('limit');
        if (token === 'true')
            return true;
        if (token === 'false')
    ...
    parseName() {
        ...
        if (!name || name.length > LIMITS.literalLength)
    ...
    parseString(quote) {
        ...
        while (this.cursor < this.source.length) {
            ...
            if (char === quote) {
                if (value.length > LIMITS.literalLength)
                    throw new Error('limit');
                return value;
            }
            if (char === '\\') {
...
function evaluateSelector(data, selector) {
    ...
    for (const step of selector.steps)
        current = applyStep(current, step, state);
...
function applyStep(nodes, step, state) {
    ...
    for (const node of nodes) {
        ...
        if (step.kind === 'property')
            pushAll(result, selectProperty(node, step.name));
        else if (step.kind === 'attribute')
            pushDefined(result, selectAttribute(node, step.name));
        else if (step.kind === 'text')
            pushDefined(result, selectText(node));
        else if (step.kind === 'index')
            pushDefined(result, Array.isArray(node) ? node[step.index] : undefined);
        else if (step.kind === 'wildcard')
            pushAll(result, selectChildren(node));
            ...
            pushAll(result, selectRecursive(node, step.target, state, 0));
        if (result.length > LIMITS.results)
            throw new Error('limit');
    }
    return result;
}
function matchesPredicate(node, predicate, state) {
    let values = [node];
    for (const step of predicate.path)
        values = applyStep(values, step, state);
...
function compareScalar(actual, operator, expected) {
    ...
    if (operator === '!=')
        return normalized !== expected;
    if (typeof normalized !== 'number' || typeof expected !== 'number')
        return false;
    if (operator === '>')
        return normalized > expected;
    if (operator === '>=')
...
function selectProperty(node, name) {
    if (Array.isArray(node))
        return [];
    if (isXmlNode(node))
        return xmlChildrenByName(node, name);
    if (node &&
        typeof node === 'object' &&
...
function selectAttribute(node, name) {
    ...
    if (name.startsWith('{')) {
        ...
        if (close > 1)
            return element.getAttributeNS(name.slice(1, close), name.slice(close + 1)) ?? undefined;
    }
...
function selectRecursive(node, target, state, depth) {
    if (depth > LIMITS.recursiveDepth)
        throw new Error('limit');
...
function visit(state) {
    state.visited += 1;
    if (state.visited > LIMITS.visitedNodes)
        throw new Error('limit');
}
...
function materialize(values, mode) {
    ...
    if (mode === 'all')
        return stableJson(values.map(toJsonValue));
    const selected = mode === 'first' ? values[0] : values.length === 1 ? values[0] : values;
    if (selected === undefined || selected === null)
        return null;
    if (Array.isArray(selected))
        return selected.length ? stableJson(selected.map(toJsonValue)) : null;
