...
O binding PowerShell DEVE suportar Windows PowerShell 5.1 e PowerShell 7.4 ou superior enquanto esses runtimes permanecerem declarados. Superfície canônica: `hasParserExpression([string] source)`, `resolveDslData([object] data, [string] path, [ScriptBlock] callback)` e `resolveParserExpression([string] source, [hashtable] options, [ScriptBlock] callback)`. `has_parser_expression`, `resolve_dsl_data` e `resolve_parser_expression` DEVEM permanecer aliases legados compatíveis; `main` e execução direta PODEM receber o primeiro argumento ou `DSL_INPUT`; dot-source NÃO DEVE autoexecutar.
